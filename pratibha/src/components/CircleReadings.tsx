...
type Offered = { _id: string; displayName: string; body: string; mine?: boolean; replyCount?: number };
type Reply = { _id: string; displayName: string; body: string; mine: boolean; parentReplyId?: string | null };

export function CircleReadings({ verseId }: { verseId: string; daily?: boolean }) {
  const { user } = useAuth();
  ...
  return (
    <view style={{ marginBottom: 24 }}>
      ...
      <text style={{ color: C.muted, fontSize: 13, lineHeight: 1.55, marginBottom: 12 }}>
        Public readings on this verse. Sit with them, then answer.
        {count ? `  ·  ${count}` : ""}
      </text>
      {offered.length === 0 ? (
        <text style={{ color: C.muted, fontSize: 14, lineHeight: 1.55 }}>
          {user ? "No one has offered a reading on this verse yet." : "Sign in to offer a reading to the circle."}
        </text>
      ) : (
        <view style={{ gap: 18 }}>
          {offered.map((reading) => (
            <view key={reading._id}>
              ...
              <ReplyBlock commentaryId={reading._id} canWrite={Boolean(user)} />
            </view>
          ))}
        </view>
      )}
...
function ReplyBlock({ commentaryId, canWrite }: { commentaryId: string; canWrite: boolean }) {
  const { httpClient } = useConvex();
    ...
    <view style={{ marginTop: 10 }}>
      {replies.map((reply) => (
        <view
          key={reply._id}
          style={{
            paddingLeft: reply.parentReplyId ? 20 : 12,
            marginBottom: 8,
            borderLeftWidth: 1,
        ...
        >
          <text style={{ color: C.faint, fontSize: 11, marginBottom: 4 }}>
            {reply.mine ? "You" : reply.displayName}
          </text>
        ...
        open ? (
          <view>
            ...
            <view style={{ flexDirection: "row", gap: 8 }}>
              <view bindtap={() => void submit()} style={{ padding: 8, backgroundColor: C.gold, borderRadius: 6 }}>
