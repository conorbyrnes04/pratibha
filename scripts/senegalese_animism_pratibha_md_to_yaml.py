#!/usr/bin/env python3
"""
Parse Pratibha markdown source for Senegalese animism into YAML units.

Usage:
  python scripts/senegalese_animism_pratibha_md_to_yaml.py <input_md> <output_dir>
"""
...
def parse_markdown(path: Path) -> list[dict]:
    ...
    for i, m in enumerate(heads):
        ...
        if "pratibha" in title.lower() and "senegalese" in title.lower():
            continue
        ...
        units.append(
            {
                "sutra_id": f"SEN_ANIM_{unit_num:03d}",
                "collection": "Senegalese Animism",
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Convert Senegalese animism Pratibha markdown to YAML files.")
    ...
    for rec in records:
        ...
        out = args.output_dir / f"sen_anim_{idx}.yml"
        out.write_text(
