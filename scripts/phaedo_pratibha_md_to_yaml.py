#!/usr/bin/env python3
"""
Parse Pratibha markdown source for Phaedo into YAML units.

Usage:
  python scripts/phaedo_pratibha_md_to_yaml.py <input_md> <output_dir>
"""
...
def parse_markdown(path: Path) -> list[dict]:
    ...
    for i, m in enumerate(heads):
        ...
        if "pratibha" in title.lower() and "phaedo" in title.lower():
            continue
        ...
        units.append(
            {
                "sutra_id": f"PHAEDO_MD_{unit_num:03d}",
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Convert Phaedo Pratibha markdown to YAML files.")
    ...
    for rec in records:
        ...
        out = args.output_dir / f"phaedo_md_{idx}.yml"
        out.write_text(
