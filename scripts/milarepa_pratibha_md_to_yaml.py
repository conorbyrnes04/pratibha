#!/usr/bin/env python3
"""
Parse Pratibha markdown for Milarepa songs into YAML units.

Usage:
  python scripts/milarepa_pratibha_md_to_yaml.py <input_md> <output_dir>
"""
...
MIL_REF_RE = re.compile(r"MIL_[A-Z_0-9]+", re.I)
CHAPTER_RE = re.compile(r"Chapter\s+([XIVLC\d]+)", re.I)
...
def _clean(s: str) -> str:
    ...
    for raw in s.split("\n"):
        ...
        if re.match(r"^\s*---+\s*$", line):
            continue
        if "Pratibha corpus entry" in line or "Pratibha — Milarepa" in line:
            continue
        lines.append(line)
    s = "\n".join(lines)
    return re.sub(r"\n{3,}", "\n\n", s).strip()
...
def _parse_refs(source: str, unit_num: int) -> tuple[str, str, str]:
    ...
    sid = ""
    m = MIL_REF_RE.search(source)
    if m:
        sid = m.group(0).upper()
    if not sid:
        sid = f"MIL_PILOT_{unit_num:03d}"
    section_label = ""
    ch = CHAPTER_RE.search(source)
    if ch:
        section_label = f"Chapter {ch.group(1).upper()}"
        ...
        units.append(
            {
                ...
                "source": source or "Milarepa, Jetsun-Kahbum Ch. X (Evans-Wentz 1928)",
...
def main() -> int:
    ...
    for rec in records:
        ...
        out = args.output_dir / f"milarepa_songs_{sid}.yml"
        out.write_text(
