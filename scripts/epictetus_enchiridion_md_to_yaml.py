#!/usr/bin/env python3
"""
Parse Pratibha markdown source for Epictetus Enchiridion into YAML units.

Usage:
  python scripts/epictetus_enchiridion_md_to_yaml.py <input_md> <output_dir>
"""
...
def _clean(s: str) -> str:
    ...
    for raw in s.split("\n"):
        ...
        if re.match(r"^\s*---+\s*$", line):
            continue
        if "Pratibhā corpus entry" in line or "Pratibha corpus entry" in line:
            continue
        lines.append(line)
    s = "\n".join(lines)
    s = re.sub(r"\n{3,}", "\n\n", s)
...
def _first_sentence_or_prefix(s: str, limit: int = 340) -> str:
    s = _clean(s)
    if not s:
        return ""
...
def _themes(*parts: str) -> list[str]:
    ...
    found = [t for t in seeds if re.search(rf"\b{re.escape(t)}\b", blob)]
    return found[:6]
...
def parse_markdown(path: Path) -> list[dict]:
    ...
    for i, m in enumerate(heads):
        ...
        if "pratibha" in title.lower() and "epictetus" in title.lower():
            continue
        start = m.end()
        ...
        greek = fields.get("original_greek", "")
        iast = fields.get("iast", "")
        translation_full = fields.get("pratibha_translation", "") or fields.get("translation", "")
        ...
        if not translation_full:
            continue

        translation = _clean(translation_full)
        long_parts: list[str] = []
        if commentary:
        ...
        units.append(
            {
                "sutra_id": f"EPI_ENC_{unit_num:03d}",
                ...
                "glossary": [],
                "themes": _themes(title, translation_full, commentary, key_terms, resonances),
                "source": source,
            }
        )
    return units
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Convert Epictetus Enchiridion markdown to YAML files.")
    ...
    for rec in records:
        ...
        out = args.output_dir / f"epictetus_enchiridion_{idx}.yml"
        out.write_text(
