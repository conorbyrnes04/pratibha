#!/usr/bin/env python3
"""
Parse Pratibha markdown source for Tao Te Ching into YAML units.

Usage:
  python scripts/tao_te_ching_md_to_yaml.py <input_md> <output_dir>
"""
...
def _themes(*parts: str) -> list[str]:
    ...
    return [t for t in seeds if re.search(rf"\b{re.escape(t)}\b", blob)][:6]
...
def parse_markdown(path: Path) -> list[dict]:
    ...
    for i, m in enumerate(heads):
        ...
        original = fields.get("original_traditional_chinese", "")
        ...
        units.append(
            {
                "sutra_id": f"TTC_MD_{unit_num:03d}",
                ...
                "glossary": [],
                "themes": _themes(title, translation, commentary, key_terms, resonances),
                "source": source,
            }
        )
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Convert Tao Te Ching markdown to YAML files.")
    ...
    for rec in records:
        ...
        out = args.output_dir / f"tao_te_ching_md_{idx}.yml"
        out.write_text(
