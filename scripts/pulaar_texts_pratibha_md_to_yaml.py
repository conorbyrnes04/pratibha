#!/usr/bin/env python3
"""
Parse Pratibha markdown source for Pulaar texts into YAML units.

Usage:
  python scripts/pulaar_texts_pratibha_md_to_yaml.py <input_md> <output_dir>
"""
...
def parse_markdown(path: Path) -> list[dict]:
    ...
    for i, m in enumerate(heads):
        ...
        units.append(
            {
                "sutra_id": f"PUL_TXT_{unit_num:03d}",
                "collection": "Pulaar Texts",
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Convert Pulaar texts Pratibha markdown to YAML files.")
    ...
    for rec in records:
        ...
        out = args.output_dir / f"pul_txt_{idx}.yml"
        out.write_text(
