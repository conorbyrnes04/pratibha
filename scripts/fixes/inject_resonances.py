#!/usr/bin/env python3
"""Inject resonances layers into canonical units WITHOUT a full re-canonicalize.

Companion to inject_key_terms.py. scripts/enrich_resonances.py writes grounded
cross-tradition resonances into the SOURCE data/yaml (flat `resonances` field).
A full canonicalize would clobber canonical-only Wave 1 enrichments, so instead:

  1. Run the canonicalizer's own normalize() on each SOURCE yaml to obtain
     exactly the resonances layer the pipeline would emit.
  2. Inject only that resonances layer into the matching data/canonical unit,
     in the spec-correct position (after key_terms, before practice/appendix),
     leaving every other field/layer byte-for-byte untouched.
  3. Keep data/canonical/<work>/*.yml and index.jsonl synchronized, with the
     same serialization + roundtrip guarantees as inject_key_terms.py.
...
# resonances, practice, appendix. resonances goes before any of these:
AFTER_RESONANCES = {"practice", "appendix"}
...
def source_resonance_layers(canon: Any) -> dict[str, dict[str, Any]]:
    """unit_id -> the resonances layer the pipeline emits from source yaml."""
    ...
    for path in canon.all_yaml_files(YAML_ROOT):
        ...
        for layer in unit.get("pratibha_layers") or []:
            if isinstance(layer, dict) and layer.get("kind") == "resonances" and layer.get("items"):
                out[uid] = layer
                break
    return out


def has_resonances(unit: dict[str, Any]) -> bool:
    return any(
        isinstance(layer, dict) and layer.get("kind") == "resonances" and layer.get("items")
        for layer in unit.get("pratibha_layers") or []
    )


def insert_resonances(unit: dict[str, Any], layer: dict[str, Any]) -> None:
    ...
    insertion = next(
        (
            ...
            if isinstance(l, dict) and l.get("kind") in AFTER_RESONANCES
        ),
        len(layers),
    )
...
def main() -> int:
    ...
    res = source_resonance_layers(canon)
    ...
    for line_number, index_unit in enumerate(index_units):
        ...
        if not layer:
            continue
        if has_resonances(index_unit):
            already.append(uid)
            continue

        next_index = copy.deepcopy(index_unit)
        next_yaml = copy.deepcopy(yaml_unit)
        insert_resonances(next_index, layer)
        insert_resonances(next_yaml, layer)
        if next_index != next_yaml:
            raise ValueError(f"{uid}: YAML/index diverged after insert")

        # Guard: re-dumping the untouched canonical unit must reproduce it byte
        # for byte, so we never churn formatting of files we are not changing.
    ...
    print(f"source units with resonance content: {len(res)}")
    print(f"already have a resonances layer (skipped): {len(already)}")
    print(f"{'would add' if args.dry_run else 'adding'} resonances layers to {len(added)} units")
    for uid in added:
        example = next(u for u in index_units if str(u["unit_id"]) == uid)
        merged = copy.deepcopy(example)
        insert_resonances(merged, res[uid])
        new_layer = next(l for l in merged["pratibha_layers"] if l.get("kind") == "resonances")
