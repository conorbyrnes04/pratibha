#!/usr/bin/env python3
"""
Parse Pratibha markdown source for Pulaar tradition into YAML units.

Usage:
  python scripts/pulaar_tradition_pratibha_md_to_yaml.py <input_md> <output_dir>
"""
...
def parse_markdown(path: Path) -> list[dict]:
    ...
    for i, m in enumerate(heads):
        ...
        if "pratibha" in title.lower() and "pulaar" in title.lower():
            continue
        ...
        units.append(
            {
                "sutra_id": f"PULAAR_{unit_num:03d}",
                "collection": "Pulaar Tradition",
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Convert Pulaar tradition Pratibha markdown to YAML files.")
    ...
    for rec in records:
        ...
        out = args.output_dir / f"pulaar_{idx}.yml"
        out.write_text(
